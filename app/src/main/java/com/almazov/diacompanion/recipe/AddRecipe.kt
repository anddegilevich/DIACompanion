...
class AddRecipe : Fragment(), FoodInMealListAdapter.InterfaceFoodInMeal {
    ...
    private fun createRecipe(idFood: Int? = null):FoodEntity {
        ...
        carbo = BigDecimal(carbo / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        prot = BigDecimal(prot / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        fat = BigDecimal(fat / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        ec = BigDecimal(ec / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        if (carbo!=0.0) gi = BigDecimal(gi / (carbo * weightSum)).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        water = BigDecimal(water / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        nzhk = BigDecimal(nzhk / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        hol = BigDecimal(hol / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        pv = BigDecimal(pv / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        zola = BigDecimal(zola / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        na = BigDecimal(na / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        k = BigDecimal(k / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        ca = BigDecimal(ca / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        mg = BigDecimal(mg / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        p = BigDecimal(p / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        fe = BigDecimal(fe / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        a = BigDecimal(a / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        b1 = BigDecimal(b1 / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        b2 = BigDecimal(b2 / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        rr = BigDecimal(rr / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        c = BigDecimal(c / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        re = BigDecimal(re / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        kar = BigDecimal(kar / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        mds = BigDecimal(mds / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        kr = BigDecimal(kr / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        te = BigDecimal(te / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        ok = BigDecimal(ok / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        ne = BigDecimal(ne / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        zn = BigDecimal(zn / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        cu = BigDecimal(cu / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        mn = BigDecimal(mn / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        se = BigDecimal(se / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        b5 = BigDecimal(b5 / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        b6 = BigDecimal(b6 / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        fol = BigDecimal(fol / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        b9 = BigDecimal(b9 / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        dfe = BigDecimal(dfe / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        holin = BigDecimal(holin / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        b12 = BigDecimal(b12 / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        ear = BigDecimal(ear / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        a_kar = BigDecimal(a_kar / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        b_kript = BigDecimal(b_kript / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        likopin = BigDecimal(likopin / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        lut_z = BigDecimal(lut_z / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        vit_e = BigDecimal(vit_e / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        vit_d = BigDecimal(vit_d / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        d_mezd = BigDecimal(d_mezd / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        vit_k = BigDecimal(vit_k / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        mzhk = BigDecimal(mzhk / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        pzhk = BigDecimal(pzhk / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        w_1ed = BigDecimal(w_1ed / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        /*op_1ed = BigDecimal(op_1ed / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        w_2ed = BigDecimal(w_2ed / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
        op_2ed = BigDecimal(op_2ed / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()*/
        proc_pot = BigDecimal(proc_pot / weightSum).setScale(1, BigDecimal.ROUND_HALF_DOWN).toDouble()
